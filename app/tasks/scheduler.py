...
def apply_scheduler_config(celery_app):
    """Apply scheduler configuration to the provided celery app"""

    # Configure beat schedule
    celery_app.conf.beat_schedule = {
        # Daily analysis at 09:00 UTC (12:00 Kyiv time)
        'daily-stock-analysis': {
            'task': 'trading_bot.daily_analysis_task',
        ...
        'weekly-summary-report': {
            'task': 'trading_bot.generate_summary_report_task',
        ...
        'monthly-summary-report': {
            'task': 'trading_bot.generate_monthly_summary_report_task',
        ...
        'system-health-check': {
            'task': 'trading_bot.health_check_task',
...
def configure_schedule_for_environment(celery_app, env: str = "development"):
...
def run_daily_analysis_at_time(celery_app, hour: int, minute: int = 0):
...
def run_summary_report_weekly(celery_app, day_of_week: int, hour: int, minute: int = 0):
...
def get_scheduled_tasks(celery_app):
    """Get list of scheduled tasks"""
    inspect = celery_app.control.inspect()
    return inspect.scheduled()


def cancel_task(celery_app, task_id: str):
    """Cancel a specific task"""
    celery_app.control.revoke(task_id, terminate=True)


def purge_queue(celery_app, queue_name: str):
...
# Health monitoring
def check_celery_health(celery_app):
...
if __name__ == "__main__":
    ...
    # For standalone execution, we need to import the celery app
    ...
    if len(sys.argv) > 1:
        ...
        if command == "health":
            health = check_celery_health(celery_app)
            print(f"Celery Health: {health}")

        elif command == "active":
            active = get_active_tasks(celery_app)
            print(f"Active Tasks: {active}")

        elif command == "scheduled":
            scheduled = get_scheduled_tasks(celery_app)
            print(f"Scheduled Tasks: {scheduled}")

        elif command == "config":
            env = sys.argv[2] if len(sys.argv) > 2 else "development"
            configure_schedule_for_environment(celery_app, env)
